...
import { Search, ChevronUp, GitFork } from 'lucide-react';
...
export default function Hierarchy({ user, isLiveMode, onPresetRegister }) {
    ...
    const fetchTree = async () => {
      ...
      try {
        ...
        if (tree) {
          ...
          setError(`Node ${rootNodeId} not found in structural layouts.`);
        }
      } catch (err) {
  ...
  const renderTreeNodes = (node, level = 1, x = 500, y = 60, widthOffset = 250) => {
    ...
    nodesList.push(
      <g key={`node-g-${node.userId}`} style={{ cursor: 'pointer' }}>
        ...
        <text x={x} y={y + 36} textAnchor="middle" fill="var(--text-white)" fontSize={10} fontWeight={600} onClick={() => handleNodeClick(node.userId)}>
          {node.userId}
        </text>
        <text x={x} y={y + 47} textAnchor="middle" fill="var(--text-grey)" fontSize={8} onClick={() => handleNodeClick(node.userId)}>
          {node.name.split(' ')[0]}
        </text>
      </g>
    );
      ...
      if (node.left) {
        ...
        nodesList.push(
          <g key={`empty-left-${node.userId}`}>
            <circle cx={x - widthOffset} cy={nextY} r={16} fill="rgba(0, 0, 0, 0.5)" stroke="var(--border-grey)" strokeWidth={1.5} />
            <circle cx={x - widthOffset} cy={nextY} r={16} fill="transparent" style={{ cursor: 'pointer' }} onClick={() => triggerAddUser(node.userId, 'Left')} />
      ...
      if (node.right) {
        ...
        nodesList.push(
          <g key={`empty-right-${node.userId}`}>
            <circle cx={x + widthOffset} cy={nextY} r={16} fill="rgba(0, 0, 0, 0.5)" stroke="var(--border-grey)" strokeWidth={1.5} />
            <circle cx={x + widthOffset} cy={nextY} r={16} fill="transparent" style={{ cursor: 'pointer' }} onClick={() => triggerAddUser(node.userId, 'Right')} />
            <g transform={`translate(${x + widthOffset - 7}, ${nextY - 7})`} pointerEvents="none">
              <path d="M7 1v12M1 7h12" stroke="var(--text-muted)" strokeWidth={2} />
            </g>
          </g>
  ...
  return (
    <div style={{ padding: '28px', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '28px', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-grey)', fontSize: '14px', marginTop: '4px' }}>
            Interact with downline network nodes and place direct sponsors onto structural branches
          </p>
        </div>
      ...
      {error && <div style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)', padding: '14px', borderRadius: '8px', color: '#f87171', marginBottom: '20px', fontSize: '14px' }}>{error}</div>}

      <motion.div 
        whileHover={{ y: -6, scale: 1.012, transition: { duration: 0.2, ease: "easeOut" } }}
        className="glass-card shifting-card" 
        style={{ padding: '24px', position: 'relative', background: '#0a0a0a', minHeight: '520px', overflowX: 'auto' }}
      >
        {treeData ? (
          ...
          <div style={{ display: 'flex', alignItems: 'center', justify: 'center', height: '420px', color: 'var(--text-muted)' }}>Loading network hierarchy...</div>
        )}
